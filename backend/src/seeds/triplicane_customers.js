...
const seed = async () => {
  try {
    ...
    const existing = await readFromCRM(
      `SELECT id FROM routes WHERE LOWER(route_name) IN ('triplicane', 'triplicane 1') LIMIT 1`
    );

    if (existing.rows.length > 0) {
      routeId = existing.rows[0].id;
      console.log(`ℹ️ Triplicane 1 route already exists: ${routeId}`);
    } else {
      ...
      console.log(`✅ Created Triplicane 1 route: ${routeId}`);
    }
