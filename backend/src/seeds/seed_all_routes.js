...
async function seedGroup(routeId, routeName, customersList) {
  console.log(`\n🌱 Seeding route: ${routeName} (UUID: ${routeId})...`);

  // Ensure route exists in routes table
  await withRetry(() => writeToCRM(
  ...
  // Determine next customer code MMxxxx
  const codeRes = await withRetry(() => readFromCRM(`SELECT customer_code FROM customers WHERE customer_code LIKE 'MM%'`));
  let maxNum = 0;
  ...
  for (const c of customersList) {
    const dup = await withRetry(() => readFromCRM(
    ...
    const result = await withRetry(() => writeToCRM(
  ...
  console.log(`✅ ${routeName} seed finished. Inserted: ${inserted}, Skipped: ${skipped}`);
}

async function run() {
  try {
    console.log('🚀 Starting batch route customer seed script...');

    // 1. Ensure all official routes exist
    ...
    // 2. Seed Nungambakkam 1
    await seedGroup('1b4a924f-1a3c-4658-9e23-a2a060917ac2', 'Nungambakkam 1', nungambakkamCustomers);

    // 3. Seed Triplicane 1
    await seedGroup('9835f558-456c-423f-985b-f21b981172d6', 'Triplicane 1', triplicaneCustomers);

    // 4. Verify customer count
    const totalRes = await withRetry(() => readFromCRM(`SELECT COUNT(*) FROM customers`));
    console.log(`\n🎉 Total customers in CRM database: ${totalRes.rows[0].count}`);
