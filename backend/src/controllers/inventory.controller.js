  ...
  try {
    ...
    if (db1ProdRes.rows.length > 0) {
      ...
      if (match) {
        ...
        // Auto-insert into DB2 "InventoryItem" table if missing
        try {
          const material = (prod.unit || '').toLowerCase().includes('packet') ? 'Packet' : 'Bottle';
          const section = category || 'AdHoc';
          await writeToApp(
            'INSERT INTO "InventoryItem" (id, name, unit, material, section) VALUES ($1, $2, $3, $4, $5)',
            [inventoryItemId, prod.name, prod.unit || '1L', material, section]
          );
          db2ItemId = inventoryItemId;
          itemName = prod.name;
          itemUnit = prod.unit || itemUnit;
          console.log(`✅ Auto-inserted missing product "${prod.name}" into DB2 "InventoryItem" table.`);
        } catch (insertErr) {
          console.warn('⚠️ Auto-insert into DB2 InventoryItem warning:', insertErr.message);
        }
      }
    }
  } catch (e) { /* silent */ }
...
// ─────────────────────────────────────────────
// GET /api/inventory/dp-audit/report — Generate DP Audit Excel Report (READ-ONLY)
// ─────────────────────────────────────────────
const generateDpAuditReport = async (req, res, next) => {
  try {
    ...
    let targetEndDate = date || istToday;
    let periodStr = '';
    let filePeriodStr = '';

    if (period === 'today') {
      periodStr = targetStartDate;
      filePeriodStr = targetStartDate;
    } else if (period === 'this_week') {
      const end = new Date(istToday);
      const start = new Date(end);
      start.setDate(end.getDate() - 6);
      targetStartDate = start.toLocaleDateString('en-CA', { timeZone: 'Asia/Kolkata' });
      targetEndDate = istToday;
      periodStr = `${targetStartDate} to ${targetEndDate} (This Week)`;
      filePeriodStr = `This_Week_${targetStartDate}_to_${targetEndDate}`;
    } else if (period === 'this_month') {
      const end = new Date(istToday);
      const start = new Date(end.getFullYear(), end.getMonth(), 1);
      targetStartDate = start.toLocaleDateString('en-CA', { timeZone: 'Asia/Kolkata' });
      targetEndDate = istToday;
      periodStr = `${targetStartDate} to ${targetEndDate} (This Month)`;
      filePeriodStr = `This_Month_${targetStartDate}_to_${targetEndDate}`;
    ...
    let selectedDps = dpRows;
    if (dpId && dpId !== 'all') {
      selectedDps = dpRows.filter(d => String(d.id) === String(dpId) || String(d.dpCode) === String(dpId) || d.name.toLowerCase() === String(dpId).toLowerCase());
      if (selectedDps.length === 0) {
    ...
    const workbook = new ExcelJS.Workbook();
    workbook.creator = 'Maram Milk ERP & CRM';
    workbook.created = new Date();

    const applyHeaderBlock = (sheet, titleName) => {
      sheet.addRow(['Maram Milk Franchise ERP']);
      sheet.addRow([`Delivery Person Audit Report — ${titleName}`]);
      sheet.addRow([`Report Period: ${periodStr}`]);
      sheet.addRow([`Generated By: ${generatedBy}`]);
      sheet.addRow([`Generated On: ${new Date().toLocaleString('en-IN', { timeZone: 'Asia/Kolkata' })}`]);
      sheet.addRow([]);
    ...
    const applyDataRowStyling = (dataRow) => {
    ...
    if (section === 'attendance') {
      const ws = workbook.addWorksheet('DP Attendance Audit');
      applyHeaderBlock(ws, 'Attendance & Absence Audit');

      const hRow = ws.addRow([
        'DP Code', 'Delivery Person Name', 'Mobile Number', 'Vehicle Number',
        'Assigned Route', 'Total Days', 'Present Days', 'Absent Days',
        'Standby Days', 'Not Marked Days', 'Attendance %', 'Account Status'
      ]);
    ...
    // SECTION 2: ROUTE REPORT
    else if (section === 'route') {
      const ws = workbook.addWorksheet('DP Route & Logistics Audit');
      applyHeaderBlock(ws, 'Route & Logistics Allocation Audit');

      const hRow = ws.addRow([
        'Date Range', 'DP Code', 'Delivery Person Name', 'Vehicle Number',
        'Assigned Route', 'Zone / Territory', 'Stops Assigned', 'Stops Completed',
        'Geofence Compliance %', 'Distance Covered (km)', 'Status'
      ]);
      styleTableHeader(hRow);
    ...
    // SECTION 3: DAILY AUDIT REPORT
    else if (section === 'daily_audit') {
      const ws = workbook.addWorksheet('DP Daily Audit');
      applyHeaderBlock(ws, 'Operational Daily Audit Log');

      const hRow = ws.addRow([
        'Operational Date', 'DP Code', 'Delivery Person Name', 'Assigned Route',
        'Milk Taken (L)', 'Milk Delivered (L)', 'Undelivered Milk (L)',
        'Petrol Paid (₹)', 'Extra Paid (₹)', 'Short Paid (₹)',
        'AdHoc Items Taken', 'AdHoc Items Delivered', 'AdHoc Items Undelivered'
      ]);
      styleTableHeader(hRow);
    ...
    // SECTION 4: DP OVERVIEW REPORT
    else {
      const ws = workbook.addWorksheet('DP Overview Master');
      applyHeaderBlock(ws, 'Delivery Person Profiles & Master Overview');

      const hRow = ws.addRow([
        'DP Code', 'Full Name', 'Mobile Number', 'Vehicle Registration No.',
        'Assigned Route / Zone', 'Account Status', 'Data Source'
      ]);
      styleTableHeader(hRow);
    ...
    // Output workbook to client
    const dpLabel = dpId === 'all' ? 'All_DPs' : (selectedDps[0]?.dpCode || 'DP');
    const reportFileName = `DP_Audit_${section.toUpperCase()}_${dpLabel}_${filePeriodStr}.xlsx`;
